...
val defaultQuestions = listOf(
    ...
    QuizQuestion(
        id = 94,
        questionText = "What is `produceState` in Jetpack Compose, and what is its primary use case?",
        answerText = "`produceState` is a composable function that converts non-Compose state into Compose `State`. [1, 2] It launches a coroutine scoped to the composition that can push values into a returned `State` object, making it easy to integrate asynchronous data sources, flows, or observables into the Compose UI. [3, 4]\n\n**How It Works:**\n\n`produceState` creates a `State<T>` object and provides a coroutine scope where you can produce values over time. [5] The coroutine is automatically launched when the composable enters the composition and canceled when it leaves. [6]\n\n```kotlin\n@Composable\nfun LoadUserData(userId: String): State<User?> {\n    return produceState<User?>(initialValue = null, userId) {\n        value = repository.loadUser(userId) // Suspending call\n    }\n}\n```\n\n**Key Characteristics:**\n\n*   **Lifecycle-Aware:** The coroutine is tied to the composable's lifecycle. When the composable is removed from the composition, the coroutine is automatically canceled, preventing memory leaks. [6, 7]\n*   **Automatic Recomposition:** Any composable reading the returned `State` will automatically recompose when the `value` property is updated inside the coroutine. [8]\n*   **Keys for Relaunching:** You can pass keys to `produceState` (like `userId` in the example). If any key changes, the old coroutine is canceled and a new one is launched with the updated values. [9, 10]\n\n**Primary Use Case:**\n\nThe primary use case is to observe and convert external data sources—such as `Flow`, `LiveData`, network calls, or database queries—into Compose `State`. [1, 11] This bridges the gap between imperative or reactive data sources and Compose's declarative state system. [12] Common scenarios include:\n\n*   Loading data from a repository or API. [13]\n*   Observing a `Flow` and exposing its latest value as `State`. [11, 14]\n*   Converting callback-based APIs into composable state. [15]\n\n**Example with Flow:**\n\n```kotlin\n@Composable\nfun ObserveUserPreferences(): State<Preferences> {\n    return produceState(initialValue = Preferences.default) {\n        preferencesFlow.collect { prefs ->\n            value = prefs\n        }\n    }\n}\n```\n\n**When to Use:**\n\nUse `produceState` when you need to transform suspend functions, flows, or other asynchronous data sources into Compose `State` that can be observed by your UI. [4, 16] It's particularly useful for migrating codebases that use `Flow` or `LiveData` to Compose, as it provides a clean bridge between reactive and declarative paradigms. [12, 17]",
        category = QuizCategory.JetpackCompose
    ),
    QuizQuestion(
        id = 95,
        questionText = "What is `snapshotFlow` in Jetpack Compose, and what is its primary use case?",
        answerText = "`snapshotFlow` is a function that converts Compose `State` objects into a Kotlin `Flow`. [1, 2] It creates a cold flow that observes reads of Compose state within its lambda block and emits whenever any of the read state values change. [3, 4]\n\n**How It Works:**\n\n`snapshotFlow` takes a lambda that reads one or more Compose state objects and returns a `Flow` that emits the result of the lambda. [5] The flow automatically tracks which state objects are read and re-executes the lambda whenever any of those state values change, emitting the new result. [4, 6]\n\n```kotlin\nval scrollState = rememberScrollState()\n\nLaunchedEffect(Unit) {\n    snapshotFlow { scrollState.value }\n        .filter { it > 100 }\n        .collect {\n            // Do something when scrolled past 100 pixels\n            showScrollToTopButton = true\n        }\n}\n```\n\n**Key Characteristics:**\n\n*   **State to Flow Conversion:** It bridges the gap between Compose's state system and Kotlin's reactive Flow API. [2, 7]\n*   **Automatic Tracking:** It automatically detects which state objects are read within the lambda and observes them. [4, 8] You don't need to manually specify dependencies.\n*   **Cold Flow:** The flow is cold, meaning it only starts observing when collected and stops when the collection is canceled. [9]\n*   **Distinct Values:** By default, it only emits when the result of the lambda changes, using structural equality. [10] Consecutive duplicate values are automatically filtered out.\n\n**Primary Use Case:**\n\nThe primary use case is to observe Compose state and react to changes using Flow operators. [1, 11] This is particularly useful when you need to:\n\n*   **Apply Flow transformations:** Use operators like `debounce`, `filter`, `map`, or `distinctUntilChanged` on state changes. [12, 13]\n*   **Combine multiple state sources:** Observe several state objects and react when any of them change. [14]\n*   **Integrate with non-Compose APIs:** Bridge Compose state to systems that expect Flow, such as repositories or use cases. [15]\n\n**Example - Debouncing Search Input:**\n\n```kotlin\nval searchQuery by remember { mutableStateOf(\"\") }\n\nLaunchedEffect(Unit) {\n    snapshotFlow { searchQuery }\n        .debounce(300)\n        .filter { it.isNotEmpty() }\n        .collect { query ->\n            // Perform search after user stops typing\n            viewModel.search(query)\n        }\n}\n```\n\n**When to Use:**\n\nUse `snapshotFlow` when you need to observe Compose state with Flow semantics, apply reactive operators to state changes, or integrate Compose state with Flow-based architectures. [16] It's the inverse of `produceState`: while `produceState` converts Flow to State, `snapshotFlow` converts State to Flow. [17]",
        category = QuizCategory.JetpackCompose
    ),
    QuizQuestion(
        id = 96,
        questionText = "What is `LaunchedEffect` in Jetpack Compose, and what is its primary use case?",
        answerText = "`LaunchedEffect` is a composable function that provides a way to run side effects in a coroutine that is scoped to the composition's lifecycle. [1, 2] It allows you to safely execute suspend functions and asynchronous operations from within a composable function. [3, 4]\n\n**How It Works:**\n\n`LaunchedEffect` launches a coroutine when the composable enters the composition. [5] The coroutine is automatically canceled when the composable leaves the composition or when any of the keys passed to `LaunchedEffect` change. [6, 7] If the keys change, the old coroutine is canceled and a new one is launched with the updated values. [8]\n\n```kotlin\n@Composable\nfun MyScreen(userId: String) {\n    var userData by remember { mutableStateOf<User?>(null) }\n    \n    LaunchedEffect(userId) {\n        userData = repository.fetchUser(userId) // Suspending call\n    }\n    \n    // Display userData...\n}\n```\n\n**Key Characteristics:**\n\n*   **Lifecycle-Aware:** The coroutine is tied to the composable's lifecycle, preventing memory leaks and ensuring proper cleanup. [6, 9]\n*   **Key-Based Restart:** You can pass one or more keys to `LaunchedEffect`. When any key changes, the effect restarts, canceling the old coroutine and launching a new one. [7, 8] This is essential for reacting to parameter changes.\n*   **Single Execution per Key Set:** If the keys don't change, the effect runs only once and continues until completion or until the composable leaves the composition. [10]\n*   **Coroutine Scope:** The lambda passed to `LaunchedEffect` is a coroutine scope, giving you access to coroutine APIs like `delay`, `launch`, and `async`. [11]\n\n**Primary Use Case:**\n\nThe primary use case is to perform side effects that need to be triggered by composition or by changes in specific values. [12, 13] Common scenarios include:\n\n*   **Loading data:** Fetching data from a repository or API when a screen is first displayed or when parameters change. [14]\n*   **Event tracking:** Sending analytics events when a screen is viewed. [15]\n*   **Observing external state:** Collecting from a Flow or observing LiveData. [16]\n*   **Triggering animations:** Starting animations based on state changes. [17]\n*   **Setting up listeners:** Registering callbacks or listeners that need cleanup. [18]\n\n**Example - Event Tracking:**\n\n```kotlin\n@Composable\nfun ScreenA(screenName: String) {\n    LaunchedEffect(screenName) {\n        analytics.logScreenView(screenName)\n    }\n    // UI content...\n}\n```\n\n**Example - Data Loading with Cancellation:**\n\n```kotlin\n@Composable\nfun SearchResults(query: String) {\n    var results by remember { mutableStateOf<List<Item>>(emptyList()) }\n    \n    LaunchedEffect(query) {\n        if (query.isNotEmpty()) {\n            results = repository.search(query) // Cancels previous search\n        }\n    }\n    // Display results...\n}\n```\n\n**When to Use:**\n\nUse `LaunchedEffect` when you need to perform side effects that involve suspend functions, require a coroutine scope, or need to react to changes in composable parameters. [19] It's the go-to API for integrating asynchronous operations into Compose's declarative paradigm. [20]\n\n**Important Note:**\nThe keys you pass to `LaunchedEffect` determine when the effect should restart. Use `Unit` or `true` as a key if you want the effect to run only once and never restart (except when the composable leaves and re-enters the composition). [21]",
        category = QuizCategory.JetpackCompose
    ),
)